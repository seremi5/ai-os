...
This rule is installed only when `aitelier.json` → `deploy.branch_strategy` is set. For projects with no staging, the deploy flow is project-specific and lives elsewhere.

## Environments

Read from `aitelier.json`:
```bash
jq -r '.project.deploy' aitelier.json
```
