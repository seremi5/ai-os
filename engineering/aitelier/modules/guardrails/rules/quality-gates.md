...
`aitelier.json`. If a command is empty for this project, the
...
- Backend tests pass: `jq -r '.project.commands.test' aitelier.json`
- Frontend tests pass (if separate test command)
- Lint clean: `jq -r '.project.commands.lint' aitelier.json`
- Typecheck passes: `jq -r '.project.commands.typecheck' aitelier.json`
...
- Changes deployed and verified on staging (if `deploy.branch_strategy`
